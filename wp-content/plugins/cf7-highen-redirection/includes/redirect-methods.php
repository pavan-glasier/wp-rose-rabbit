...
function cf7_highen_forms_enabled() {
	...
	foreach($posts_array as $post) {
		...
		if ($enable == "1") {
			...
			if($cf7_highen_redirect_type == 'url'){
				...
				$cf7_highen_tab = get_post_meta( $post_id, "_cf7_highen_tab", true);
			}
			else{
				$cf7_highen_url = get_post_meta( $post_id, "_cf7_highen_page", true);
				$cf7_highen_tab = get_post_meta( $post_id, "_cf7_highen_page_tab", true);
			}
			$enabled[] = '|'.$post_id.'|'.$cf7_highen_redirect_type.'|'.$cf7_highen_url.'|'.$cf7_highen_tab.'|';
		}
	}
